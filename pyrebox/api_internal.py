...
def bp_func(*arg):
    ...
    bp_num = arg[0]
    #The first argument of insn begin and mem write/read callbacks should always be cpu_index
    cpu_index = arg[1]
...
def insert_new_module_from_base(base,size,pid,pgd,name,fullname):
    import windows_vmi
    windows_vmi.windows_insert_module_from_base(base,size,pid,pgd,name,fullname)
    return True
...
def register_callback(module_hdl, callback_type, py_callback,first_param=None,second_param=None):
    ...
    if first_param is None:
        ...
        return c_api.register_callback(module_hdl,callback_type,py_callback,first_param)
    else:
        return c_api.register_callback(module_hdl,callback_type,py_callback,first_param,second_param)
