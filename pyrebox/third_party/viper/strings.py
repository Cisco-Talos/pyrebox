...
class Occurrence():
    def __init__(self,pos,string,is_host):
        self.pos = pos
        self.string = string
...
class Strings():
    ...
    def strings(self,base,data):
        ...
        while m is not None:
            ...
            results.append(Occurrence(base + start + m.start(),string,self.is_host(string)))
            start += m.end()
            data = data[m.end():]
            m = re.search(regexp,data)
        return results
